...
{
	this->color = color;
	this->shaderType = FULL_VOLUME;
	this->volumeType = HETEROGENEOUS;
	this->densitySource = VDB_DENSITY;
...
void IsosurfaceMaterial::renderInMenu() 
{
	...
	if (ImGui::Combo("Shader Type", (int*)&shaderType, "Absorption\0Absorption-Emission\0Full Volume\0")) {
		...
		if (shaderType == FULL_VOLUME and this->densitySource == VDB_DENSITY && this->texture) {
			this->shader = Shader::Get("res/shaders/basic.vs", "res/shaders/isosurface.fs");
			// automatic changes to make better the visualization
			this->volumeType = HETEROGENEOUS; // as the homogeneous doesn't have emission light 
			Application::instance->ambient_light = glm::vec4(0.1f);
